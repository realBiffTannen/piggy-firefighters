#!/usr/bin/env node
// REGRESSION (from the port review repro qa/smoke/review/shutter_race.mjs): SceneShutter failsafe misfire. Drive the bay door directly through the game's event emitter:
...
const { chromium } = (() => { for (const c of [process.env.PLAYWRIGHT_MODULE, 'playwright', '/opt/node22/lib/node_modules/playwright']) { if (!c) continue; try { return require(c); } catch { /* next */ } } throw new Error('playwright not found'); })();
...
const result = await page.evaluate(async (stall) => {
	...
	const outcome = await Promise.race([closing, new Promise((r) => setTimeout(() => r('TIMEOUT (120 s)'), 120000))]);
...
const ok = result.some((l) => l.includes('close(outro) settled')) && !errors.length;
console.log(ok ? 'PASS shutter_race' : 'FAIL shutter_race');
