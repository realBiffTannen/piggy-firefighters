...
await page.goto(`${GAME}?sessionID=diag2-${FIX}-${Date.now()}&rgs_url=${RGS}&device=desktop&fixture=${FIX}`, { waitUntil: 'domcontentloaded' });
...
const snap = () => page.evaluate(() => ({ rescue: window.__rvRescue.stateRescue.active, covered: window.__rvScene.stateScene.covered, busy: window.__rvScene.stateScene.busy, card: window.__pffCard ? { live: window.__pffCard.live, kind: window.__pffCard.kind } : null, finals: window.__pffFinalWins ?? [], log: window.__rvLog.slice(-8) }));
...
while (Date.now() - t0 < 1700000) {
  ...
  if (closeSeenAt && !poked && Date.now() - closeSeenAt > 60000 && s.rescue) {
    poked = true;
    out.stuck = s;
    await page.screenshot({ path: new URL('./diag2_stuck.png', import.meta.url).pathname });
    await page.evaluate(() => { window.__rvLog.push({ t: Math.round(performance.now()), poke: 'dismissFeatureCard' }); window.__rvDir.dismissFeatureCard(); });
  }
  await sleep(closeSeenAt ? 3000 : 8000);
}
await page.screenshot({ path: new URL('./diag2_end.png', import.meta.url).pathname });
writeFileSync(new URL('./diag2.json', import.meta.url), JSON.stringify(out, null, 1));
await browser.close();
