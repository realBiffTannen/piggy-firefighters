...
## 2026-09-25 — THUMBNAILS LANDED `ca0388c` · ALL FOUR RIGS' MASTERS/SHEETS LANDED (r1) · art lane complete

**PF-THUMB-01 delivered** (`thumbnail/`): `foreground_3_4.png` + `background_3_4.png` + `preview_3_4.png` at
1536×2048 and `foreground_16_9.png` + `background_16_9.png` + `preview_16_9.png` at 2048×1152 — generated at those
NATIVE sizes (the API accepted them; responses recorded), one Chief Hamm (hero of record, salute pose T3 chosen per
aspect from three candidates viewed at 120×160 / 320×180, one refinement each), foregrounds genuine RGBA with
solidified interiors (alpha 0–255), backgrounds opaque RGB in exactly three tones of one hue (dominant **#26365C**,
light #42578A, dark #1E2C4F; top-3 colours cover 98.6% / 99.2%), previews flattened, quiet title space above /
left with the hero toward the right; `thumbnail/source-record.json` holds model, prompts, refs, cost (10 calls,
$3.80), background hex, sha256 of every final file, alpha inspection, review renders and the written review;
`make_thumbnails.py` 14/14 and your `validate_thumbnail.py` **PASS 8/8**. Candidates under `thumbnail/source` and
`thumbnail/review`. Your `thumbnail/instructions.md` delivery record is yours to fill from that JSON. Generation
owner of record: Claude (art lane); no further tile generation planned.
**Decision on the "13":** the helmet shield carries the locally lettered "13" emblem in the tiles, per the shield
rule — it is the character's identity mark, not a title/logo/claim/watermark; if the owner reads the brief's
typography ban as covering emblems, a blank-shield variant rebuilds for free (skip the lettering step in
`thumbnail/source/prep_hero.py`) — say so and I post it.

**Rigs r1:** `art-src/animation/parts/{pf_chief,pf_rookie,pf_dog,pf_rescued}/` — masters, same-framing hidden-surface
edits, sheets, r1 pieces, `registration.json`, `anchors.json` (head_top, nozzle_tip, grip_l/r, sheet_l/r, feet),
`INVENTORY.md` + `REGISTRATION.md`; 47 calls. Known r1 limits (all free fixes, none paid): registration good to a
few px (Spine nudge), coat tails by hem / raised arms by shoulder are approximate, rescued skins are whole re-dressed
figures (cut into slots with the template masks), dog legs are one layer, chief head_blank crown lumpy under the
helmet, some eye pieces carry brows. **The r2 re-cut (fixed grid, labelled contact sheets reviewed by eye, eyes
rule, legs doubling, helmet_front seams, atomic writes) starts now**; "CHIEF PIECES r2" follows.

**Audit** (`art-src/ART_REPORT.md`, agent PASS with warnings, no human sign-off): 0 sha256 matches against the three
donor repos across 665 files; all sizes/alpha/manifests pass; symbols legible at 85/40 px. One flagged item outside
the art folders: `static/assets/hud/*.svg` and `static/assets/fonts/*` are byte-identical to LUCKY's. Decision: the
HUD SVGs and Lilita/Inter faces are the STUDIO HUD KIT (shared by every studio title, not game art) and stay; the
`interGold` bitmap font was generated by LUCKY and gets regenerated in this title's brass palette in Phase B.

---

## 2026-09-25 — AUDIO BUILD LANDED `55e615e` · five redraws issuing · seam notes for Phase B

The offline build + adversarial review + fix round are complete: all 232 cue ids ship in both codecs under
`apps/piggy_firefighters/static/assets/audio/piggy_firefighters/` (464 files, 30 MB; sha static == runtime), donor
audio purged; `cueManifest.ts` regenerated (232 cues, donor export shape — durations changed, e.g. alarm lands
0.9→1.54 s, blaze mults 0.95→1.6 s, reel stops 0.48→0.6 s; the runtime must read `durationMs` from the manifest).
Measured: true peak ≤ −1.18 dBTP both codecs, 14/14 loop seams clean, 11/11 beds whole-bar grid-exact, bed LUFS on
target, rung beds strictly rising, 16/16 reward chains rising in stereo / mono / phone-proxy, ladders C4-D4-E4-G4-A4
reel stops, alarm chords unresolved until `trigger_fanfare`, ta-da C4…E5, count-up C5→D7. One gate still failing:
`base_loop_a` chug 3.77 (a draw defect) — its redraw plus four more (inferno bed key, backdraft layer, BIG WIN
