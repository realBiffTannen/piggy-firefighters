...
- **Protected families.** Every write is guarded, and nothing ever goes into
  `static/assets/{hud,fonts,audio,spine}` (the fonts dir holds this title's own `interGold` bitmap font, built by `make_inter_gold_font.py` below, which is the one tool allowed to write there).
...
| `derive_winrungs.py [titles signs pieces coins fx]` | `titles`: BIG / HUGE / MEGA / EPIC / MAX WIN PNGs + outlined/live SVGs in `art-src/winrungs/titles/`. `signs`: `winrungs/signs/<rung>.webp` 1200×728 on the flat_manifest geometry (board 1139.4×364.3 at (600,338.3), plank 901.7×187.2 at (600,634.2); `--plank-w 820` is the runtime's text fit), built from one plaque sheet (5 boards on the top row, 2 bars below, `--crest epic,max`) and hung on brass chains; it writes `flat_manifest.json`. `pieces`: 10 items from one 5×2 sheet, each a 2D spin of 24 frames in 128 px cells on 8 columns (1024×384) + json, plus a 96 / 48 px blind-test sheet. `coins`: `winrungs/coins/coin_sheet.webp` 1024×512 (32 frames) + `coin_sheet.json` (animation `coin`), in its own dir so it never collides with the 24-frame `pieces/coin_sheet.webp`. `fx`: this title's own procedural additive textures (flare 1024×128, water-ripple ring 512, 8-point glint 256, steam puff 256, searchlight wedge 256×512). |
...
## Registration notes (the frontend consumes these families directly)

`apps/piggy_firefighters/src/game/assets.ts` / `assetsScene.ts` register every family above by its delivered path
(symbols `sprites/symbolsCartoon/sym_<ID>.webp` + `sprites/symbolsCartoonTall/symT_<ID>.webp`, signs
`winrungs/signs/<rung>.webp`, pieces `winrungs/pieces/<name>_sheet.webp`, coins `winrungs/coins/coin_sheet.json`,
plates `environment/<mood>_<orient>.webp`, the frame / cells `ui_scene/*`, the Rescue block `features/rescue/*`), and
`src/game/artMeta.ts` imports the meta JSON beside the art (`frame.meta.json`, `cells.meta.json`, `rooms.meta.json`,
`props.meta.json`, `flat_manifest.json`, `maxwin/manifest.json`, the symbol manifests) for every rect a component
needs. The full key -> file -> consumer table is docs/FRONTEND_NOTES.md §4. A derive tool that changes a path or a
rect changes that table too.

## `make_inter_gold_font.py` (the one tool that writes into `static/assets/fonts`)

Builds `static/assets/fonts/interGold/interGold.{xml,png}`, the BMFont family `gold` the runtime's win numbers use,
from Inter Black (`static/assets/fonts/InterVariable.woff2`, OFL) in THIS title's palette: ink `#3B2313`, brass face
`#E3AE3C`, one hard lighter band `#F5D26A` on the upper half, deeper brass side `#B8862B` (theme bible §1 brass gold,
`art-src/ART_HERO.md` shield rule). Same 113-glyph set, em 184, line height 266 / base 194 and XML layout as before, so
nothing in the app changes. Specimen: `qa/art/interGold_specimen.png`. Adapted from LUCKY's tool (reference only; the
donor font bytes no longer ship — `verify_art.py` fails a byte match with any donor file).
