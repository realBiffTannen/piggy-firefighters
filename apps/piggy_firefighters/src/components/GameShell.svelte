<script lang="ts">
	/**
	 * The GAME SHELL: everything the root layout used to mount for the real game — publisher bumper,
	 * game context, Authenticate/LoadI18n, the Pixi <Game />, the studio HUD, Splash and NoSelect.
	 * routes/+layout.svelte imports this LAZILY and only outside the dev-only /rigs viewer route, so
	 * the viewer never mounts a game canvas (Codex viewer blocker, 2026-09-25). The build never ships
	 * /rigs (tools/build_dist.sh moves the route out), so production always takes this path.
	 */
	...
	// loaders: a branded DOM overlay with a real progress bar, a press-anywhere gate that
	// doubles as the audio-unlock gesture, and the hand-off that reveals the game + studio HUD.
	import Splash from './Splash.svelte';
	import NoSelect from './NoSelect.svelte';
