 ...
 * `...&fixture=golden_four`) tells the mock RGS to return that
 * deterministic fixture book on the following play(s) and to charge that
 * fixture's mode cost. The mock reads them from `server/fixtures/` (the
 * COPY/FE lane's copy of Codex's LUCKY production fixtures, all nine modes,
 * 25,000x cap; see its index.json) unless FIXTURES_DIR says otherwise. This is a
 ...
 * the session then replays that book, which is exactly what iterating on the
 * build scene wants. Append `&fixture=off` (or remove the param and reload) to
 * clear it.
 */
